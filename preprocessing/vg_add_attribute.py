...
image_data = json.load(open('VG/image_data.json'))
...
original_objects = json.load(open("VG/objects.json"))
original_attributes = json.load(open("VG/attributes.json"))
original_attribute_synsets = json.load(open("VG/attribute_synsets.json"))
    ...
    for img in all_attributes:
        ...
        for anno in img_annos:
            if 'attributes' in anno:
                ...
                # item = anno['attributes']
                # if item in attribute_counts:
                #     attribute_counts[item] = attribute_counts[item] + 1
                # else:
                #     attribute_counts[anno['attributes']] = 1
                item = anno['attributes']
                if item in attribute_counts:
                    attribute_counts[item] = attribute_counts[item] + 1
                else:
...
"""
def merge_synset_attribute(mapping_dict, attribute_counts_list, TOPK=2):
...
attribute_counts_list.sort(key=lambda v:v[0])
...
cared_mapping, purged_atts, selected_atts = merge_synset_attribute(mapping_dict, attribute_counts_list, TOPK=22)
...
with open('VG-SGG-dicts-with-attri.json', 'w', encoding='utf8') as outfile:  
    json.dump(vg_sgg_dicts, outfile, ensure_ascii=False)
        ...
        for obj_idx in range(ith_s, ith_e+1):
            ...
            for match_idx in np.where(match_idxs)[0].tolist():
                if 'attributes' in img_atts[match_idx]:
                    # for attri_name in str(img_atts[match_idx]['attributes']):
                    #     att_idx = attribute_to_index(attri_name, attri_to_idx, cared_mapping)
                    #     if att_idx != 0:
                    #         obj_att_set.append(att_idx)
                    attri_name = str(img_atts[match_idx]['attributes'])
                    att_idx = attribute_to_index(attri_name, attri_to_idx, cared_mapping)
                    if att_idx != 0:
                        obj_att_set.append(att_idx)
            # remove duplicate
...
def draw_boxes(image_id, boxes):
    pic = Image.open('/Users/liyingxuan/Desktop/data_tools/VG/images/{}.jpg'.format(image_id))
...
def show_box_attributes(image_data, vg_sgg, obj_attributes, vg_sgg_dicts, img_idx=None):
    ...
    filename = '/Users/liyingxuan/Desktop/data_tools/VG/images/{}.jpg'.format(str(image_data[img_idx]['image_id']))
