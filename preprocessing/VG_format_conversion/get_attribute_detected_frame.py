...
data = pd.read_csv('./detected_frame.csv')
...
for i, manga_name in enumerate(manga_list):
    ...
    for annotation in annotations:
        ...
        # find frame
        frame_coords = {}
        rows = data.loc[data['index']==int(images['image_id'])]
        for index, row in rows.iterrows():
            frame_coords[index] = {'xmin':row['xmin'],'ymin':row['ymin'],'xmax':row['xmax'],'ymax':row['ymax'],'order':row['order']}
                ...
                if annotation_type in {'body'}:
                    ...
                    for frame_coord in frame_coords:
                        if frame_coords[frame_coord]['xmin']<x_center<frame_coords[frame_coord]['xmax'] and frame_coords[frame_coord]['ymin']<y_center<frame_coords[frame_coord]['ymax']:
                            objects['attributes'] = int(frame_coords[frame_coord]['order'])
        ...
        images['attributes'] = object_list
        image_list.append(images)
    
output = json.dumps(image_list, ensure_ascii=False)
with open('./VG/attributes.json', 'w') as f:
        f.write(output)
