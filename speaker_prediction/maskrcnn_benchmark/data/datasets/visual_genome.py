...
class VGDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, split, img_dir, roidb_file, dict_file, image_file, transforms=None,
                ...
                filter_duplicate_rels=False, filter_non_overlap=False, flip_aug=False, custom_eval=False, custom_path=''):
        """
        ...
        if self.custom_eval:
            ...
            self.split_mask, self.gt_boxes, self.frame_boxes, self.gt_classes, self.gt_attributes, self.relationships = load_graphs(
    ...
    def get_groundtruth(self, index, evaluation=False, flip_img=False):
        ...
        # recover original frame box from BOX_SCALE
        frame_box = self.frame_boxes[index] / BOX_SCALE * max(w, h)
        frame_box = torch.from_numpy(frame_box).reshape(-1, 4)  # guard against no boxes
        if flip_img:
            new_frame_xmin = w - frame_box[:,2]
            new_frame_xmax = w - frame_box[:,0]
            frame_box[:,0] = new_frame_xmin
            frame_box[:,2] = new_frame_xmax
        ...
        target.add_field("frames", frame_box)
...
def get_VG_statistics(img_dir, roidb_file, dict_file, image_file, must_overlap=False):
    train_data = VGDataset(split='train', img_dir=img_dir, roidb_file=roidb_file, 
                        dict_file=dict_file, image_file=image_file, num_val_im=50, 
...
def load_graphs(roidb_file, split, num_im, num_val_im, filter_empty_rels, filter_non_overlap):
    ...
    frame_boxes = roi_h5['frame_{}'.format(BOX_SCALE)][:]
    ...
    frame_boxes[:, :2] = frame_boxes[:, :2] - frame_boxes[:, 2:] / 2
    frame_boxes[:, 2:] = frame_boxes[:, :2] + frame_boxes[:, 2:]
    ...
    for i in range(len(image_index)):
        ...
        frame_i = frame_boxes[i_obj_start : i_obj_end + 1, :]
    ...
    return split_mask, boxes, frames, gt_classes, gt_attributes, relationships
