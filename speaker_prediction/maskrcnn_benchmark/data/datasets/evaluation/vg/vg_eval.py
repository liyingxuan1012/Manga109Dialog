...
from maskrcnn_benchmark.data.datasets.evaluation.vg.sgg_eval import TextRecall, SGRecall, SGNoGraphConstraintRecall, SGZeroShotRecall, SGNGZeroShotRecall, SGPairAccuracy, SGMeanRecall, SGNGMeanRecall, SGAccumulateRecall
...
def do_vg_evaluation(
    ...
    if "relations" in iou_types:
        ...
        # text recall
        eval_text_recall = TextRecall(result_dict)
        eval_text_recall.register_container(mode)
        evaluator['eval_text_recall'] = eval_text_recall
        ...
        # print result
        result_str += eval_text_recall.generate_print_string(mode)
    ...
    if "relations" in iou_types:
        ...
        # return float(np.mean(result_dict[mode + '_text_recall'][100]))
...
def evaluate_relation_of_one_image(groundtruth, prediction, global_container, evaluator):
    ...
    evaluator['eval_text_recall'].calculate_recall(global_container, local_container, mode)
