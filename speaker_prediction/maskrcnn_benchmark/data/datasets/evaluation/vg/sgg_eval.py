...
class TextRecall(SceneGraphEvaluation):
    def __init__(self, result_dict):
        super(TextRecall, self).__init__(result_dict)
        

    def register_container(self, mode):
        self.result_dict[mode + '_text_recall'] = {20: [], 50: [], 100: []}

    def generate_print_string(self, mode):
        result_str = 'SGG eval: '
        for k, v in self.result_dict[mode + '_text_recall'].items():
            result_str += '  t-R @ %d: %.4f; ' % (k, np.mean(v))
        result_str += ' for mode=%s, type=Text Recall(Main).' % mode
        result_str += '\n'
        return result_str
    ...
    def calculate_recall(self, global_container, local_container, mode):
        ...
        gt_rels = [(i[0], i[1], i[2]) for i in gt_rels.tolist()]
        pred_rels = [(i[0], i[1], i[2]) for i in pred_rels.tolist()]
        text_list = [gt_rel[1] for gt_rel in gt_rels]   # multiple speakers
        pred_list = [pred_rel[1] for pred_rel in pred_rels]
        indices = []
        ...
        for k in self.result_dict[mode + '_text_recall']:
            # the following code are copied from Neural-MOTIFS
            rec_i = float(len(pred_correct)) / float(len(gt_rels))
            self.result_dict[mode + '_text_recall'][k].append(rec_i)
