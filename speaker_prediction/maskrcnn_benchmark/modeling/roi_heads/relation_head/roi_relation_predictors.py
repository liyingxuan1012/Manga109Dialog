...
@registry.ROI_RELATION_PREDICTOR.register("Baseline")
class Baseline(nn.Module):
    ...
    def forward(self, proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, logger=None):
        ...
        for pair_idx, obj_pred in zip(rel_pair_idxs, obj_preds):
            prod_reps.append( torch.cat((roi_features[pair_idx[:,0]], roi_features[pair_idx[:,1]]), dim=-1) )
...
class MotifPredictor(nn.Module):
    ...
    def forward(self, proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, logger=None):
        ...
        tail_reps = tail_rep.split(num_objs, dim=0)
        obj_preds = obj_preds.split(num_objs, dim=0)
        # attributes = cat([proposal.get_field("attributes").long() for proposal in proposals], dim=0)
        # attributes = attributes.split(num_objs, dim=0)
        # frames = cat([proposal.get_field("frames").long() for proposal in proposals], dim=0)
        # frames = frames.split(num_objs, dim=0)
        ...
        # panel_order_diffs = []
        # panel_crood_diffs = []
        # for pair_idx, head_rep, tail_rep, obj_pred, attribute in zip(rel_pair_idxs, head_reps, tail_reps, obj_preds, attributes):
        # # for pair_idx, head_rep, tail_rep, obj_pred, attribute, frame in zip(rel_pair_idxs, head_reps, tail_reps, obj_preds, attributes, frames):
        #     prod_reps.append( torch.cat((head_rep[pair_idx[:,0]], tail_rep[pair_idx[:,1]]), dim=-1) )
        #     pair_preds.append( torch.stack((obj_pred[pair_idx[:,0]], obj_pred[pair_idx[:,1]]), dim=1) )
            
        #     # panel order information
        #     k = attribute[pair_idx[:,0]][:,0] - attribute[pair_idx[:,1]][:,0]
        #     # x = (frame[pair_idx[:,0]][:,0] + frame[pair_idx[:,0]][:,2]) / 2 - (frame[pair_idx[:,1]][:,0] + frame[pair_idx[:,1]][:,2]) / 2
        #     # y = (frame[pair_idx[:,0]][:,1] + frame[pair_idx[:,0]][:,3]) / 2 - (frame[pair_idx[:,1]][:,1] + frame[pair_idx[:,1]][:,3]) / 2

        #     panel_order_diffs.append( 1 / (2 + abs(k)) )
        #     # panel_crood_diffs.append( 1 / (2 + abs(x) + abs(y)) )
        # prod_rep = cat(prod_reps, dim=0)
        # pair_pred = cat(pair_preds, dim=0)
        # panel_order_diff = cat(panel_order_diffs, dim=0).reshape(-1,1)
        # # panel_crood_diff = cat(panel_crood_diffs, dim=0).reshape(-1,1)
        # # panel_order_diff = self.w1 * panel_order_diff + self.w2 * panel_crood_diff
        # # panel_order_diff = self.w1 * panel_order_diff * panel_crood_diff
        ...
        #         prod_rep = prod_rep * self.up_dim(union_features)
        #     else:
        #         prod_rep = prod_rep * union_features * panel_order_diff
